...
title: | 
  ...
  O que é estatística? Unidade, dado estatístico e variáveis
...
Neste momento, abordaremos uma parte introdutória da disciplina, voltada para a diferenciação e conhecimento da estatística descritiva e inferencial, dados estatísticos e variáveis.  
Como objetivo aqui é a integralização do conhecimento estatístico ao software **R**, tentaremos sempre combinar exemplos com as funcionalidades do software.  
...
Considere o que já vimos sobre variáveis numéricas e categóricas. No R as variáveis numéricas contínua e discreta são categorizadas pelo tipo *Numeric* e são divididas respectivamente como *double* (dbl) e *integer* (int). As mesmas possuem basicamente a mesma descrição dentro e fora do software R.  
...
No R, para definirmos uma varíavel, utilizamos o sinal `=` ou `<-`, como por exemplo:  
...
A estatística descritiva, como seu próprio nome já nos indica, visa descrever e sumarizar os dados. A sumarização desses dados podem ser apresentadas tando em forma de gráfico, como em formas numéricas através de tabelas. Assim, consideramos então que o principal objetivo da estatística descritiva é realzar a s´ntese de dviersos valores de uma mesma natureza, o que permite apresentar uma visão geral desses valores através de três principais tipos: *Tabelas, Gráficos e medidas descritivas*  
  
## Como construir uma tabela no R  
  
Para construir uma tabela utilizando a IDE do **RStudio**, primeiro é necessário carregar o banco de dados que se irá trabalhar.  
  
### Banco de dados

No R, os bancos de dados (tabelas) são utilizados para armazenar dados ou informações estruturadas, que permite o gerenciamento de toda informação ali coletadas. Eles podem ser divididos em Banco de dados relacionais e não relacionais. A partir destes bancos de dados, que se realiza a base de uma análise de dados.
...
### Como carregar o banco de dados: Selecionando o diretório de trabalho

Para carregar um banco de dados no RStudio, deve-se seguir alguns passos:\
Primeiro é preciso estabelecer qual o diretório de trabalho (working dicerctory).  
O diretório de trabalho é a pasta a qual serão salvos / carregados os arquivos.\
Para isso, vá na opção *Session* no canto superior direito de seu RStudio e selecione a opção *Set Working Directory*. Nesta aba terão duas opções, a primeira (To source File Location) fará com que o R estabeleça como diretório de trabalho, a pasta na qual você está trabalhando, portanto, caso seu **script** esteja na área de trabalho, por exemplo, é lá que será salvo.\
...
Existe outra maneira de selecionar o diretório de trabalho, esta é feita a partir do comando `setwd()`, que significa "select working directory". a partir dela você escreverá entre áspas, no parênteses, o diretório selecionado, como por exemplo: `setwd("C:/Users/Victor/Desktop")`.  
...
Uma tabela de frequência, de forma resumida, se trata de uma maneira para organizar os dados de ma tabela de forma que mostre a frequência de cada um dos diferentes valores de uma variável. Em uma tabela de frequência, é recorrente serem inclusas as frequências relativas, acumuladas e frequencia relativa acumulada. 
...
Para criar uma tabela de frequência contínua no R, devemos seguir passos diferentes pois, precisamos definiir categorias para criar esta tabela pois o R não consegue determinar cada valor diferente como uma categoria, visto que existem valores decimais diferentes. Para isso precisamos primeiro determinar a amplitude dos dados. 
...
Seguindo a regra sturges e de como queremos o mínimo e o máximo de nossa amplitude, utilizamos a função ``table(cut())``. A função ***cut*** servirá para separar a nossa variável em classes diferentes de valores. Nossa função será ``table(cut(dados$Salario, seq(0, 6, l = 7)))``, observe que utilizamos também a função ***seq*** que nos serve para gerar uma sequência do que queremos e o parâmetro ***l*** que nos indicará quantas classes devem existir.  
No código anterior, com a regra Sturges, nos foi determinado que a quantidade adequada de intervalos seria 6, portanto adicionaremos uma unidade a mais pois, se essa função fosse aplicada com apenas 6, o R criaria uma classe a menos por isso, sempre colocamos uma unidade a mais. 
...
Dentre os tipos de gráficos mais utilizados e conhecidos, temos os histogramas (barras) e gráficos de setores (pizza). Nós aprenderemos a seguir como realizar a construção destes gráficos utilizndo a IDE básica do R e posteriormente ampliaremos o conhecimento a partir de alguns pacotes para construções de gráficos.  
...
Como forma de demonstrar diferentes formas de se construir tabelas e gráficos, nós iremos apresentar uma forma diferente para criar a tabela e o gráfico. Optaremos por a forma abaixo pois, as funções do R possuem passos já programados de como realizar determinados cálculos, e o que queremos aqui é mostrar como se faz uma tabela de frequência utilizando parâmetros que geralmetne são vistos em uma aula de bioestatística.  
...
O primeiro passo para a criação desta tabela de frequências, que simula a quantidade de albumina no plasma sanguíneo de 25 pacientes, foi criar o vetor ``qtd_albumina`` que agrupassem os dados.  
...
Para a construção do histograma, nós utilizamos a função ``hist()``. Esta função é usada para criar histogramas e atribuimos a ela alguns parâmetros, como é possível ver no código. Primeiramente precisamos atribuir a função hist, a nossa variável com os valores da tabela (qtd_albumina). Em seguida é preciso definir o título, para isso utilizamos o parâmetro ``main = `` que define o título principal do histograma.  
É preciso também definir os rótulos do eixo X e eixo Y, através dos parâmetros ``xlab = `` e ``ylab = ``, que irão servir para descrever a representação dos eixos X e Y.  
O Parâmetro ``col = `` define a cor das barras do histrograma, enquanto que o parâmetro ``border = `` define a cor das bordas do histograma.  
Para definir os limites das classes do histograma, utilizamos o parâmetro ``breaks = `` e especificamos ele com a variáel que criamos para isso.  
...
Em ``axis(2, at=seq(0,max(tabela_freq_albumina),2))``, nós adicionamos o eixo y no gráfico, indicando através do argumento **2**. Aqui foi utilizado ``seq(0,max(tabela_freq_albumina),2)`` para criar uma sequências de valores com crescimento de 2 a partir de 0 até o valor máximo da tabela de frequências.
...
Na IDE base do R, podemos utilizar a função ``pie()`` para criar um gráfico de setores.    
...
Neste tutorial, aprendemos algumas funções básicas do R, seguindo o conhecimento de estatística. Vale lembrar que todos os códigos apresentados aqui são apenas uma das várias maneiras de se construir tabelas e gráficos. Com o tempo e prática, é possível customizar cada vez mais esses códigos a partir de diferentes pacotes.  
No R o pacote **ggplot2** traz várias customizações e novidades para se construir gráficos, no decorrer dos tutoriais tentaremos abordar algumas funções deste pacote.  
Nos próximos tutoriais nos aprofundaremos sobre o conhecimento de medidas de posição e dispersão. Além de outros conhecimentos.
