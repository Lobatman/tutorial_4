...
Este tutorial faz parte de uma sequência de tutoriais que buscam trazer a integração da programação ao ambiente acadêmico, mais especificamente, o conhecimento do Software **R** com a disciplina de Bioestatística.  
Neste momento, abordaremos uma parte introdutória relacionado a Bioestatística, voltada para a diferenciação e conhecimento da estatística descritiva e inferencial, dados estatísticos e variáveis.  
...
Como visto no tutorial anterior, na estatística, variáveis são atributos que podem ser observados e mensurados em uma unica unidade, podendo variar de uma unidade para outra. Por exemplo:  
...
A estatística descritiva, como seu próprio nome já nos indica, visa descrever e sumarizar os dados. A sumarização desses dados podem ser apresentadas tando em forma de gráfico, como em formas numéricas através de tabelas. Assim, consideramos então que o principal objetivo da estatística descritiva é realizar a síntese de dviersos valores de uma mesma natureza, o que permite apresentar uma visão geral desses valores através de três principais tipos: *Tabelas, Gráficos e medidas descritivas*  
  
## Como construir uma tabela no R  
  
Para construir uma tabela utilizando a IDE do **RStudio**, podemos cria-la diretamente dentro do software R, ou podemos carregar um banco de dados direto de um diretório.  
...
Como já vimos, primeiro é preciso estabelecer qual o diretório de trabalho (working dicerctory).  
...
Outra maneira também já vista, para selecionar o diretório de trabalho, é feita a partir do comando `setwd()` ("select working directory"). A partir dela você escreverá entre áspas, no parênteses, o diretório selecionado, como por exemplo: `setwd("C:/Users/Victor/Desktop")`.  
